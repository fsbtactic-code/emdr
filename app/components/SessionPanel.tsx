...
import { Users, Copy, Check, Radio, Power } from 'lucide-react';
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar px-5 py-5 flex flex-col gap-5">
            {!roomId || !isHost ? (
              ...
              <>
                <div className={`flex items-center gap-2.5 px-4 py-3 rounded-2xl border ${clientActive ? 'bg-emerald-500/15 border-emerald-500/20 text-emerald-200' : 'bg-white/[0.03] border-white/[0.06] text-white/45'}`}>
                ...
                <div className="flex flex-col gap-2">
                  <SectionLabel>{t.sigHeading}</SectionLabel>
                  {!incomingSignal ? (
                ...
                <div className="flex flex-col gap-2">
                  <SectionLabel>{t.sessClientLink}</SectionLabel>
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={copy}
                      className={`shrink-0 px-3 rounded-xl border flex items-center gap-1.5 text-[12px] font-semibold transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500/50 focus-visible:ring-offset-1 focus-visible:ring-offset-zinc-950 ${copied ? 'bg-emerald-500/15 border-emerald-500/20 text-emerald-300' : 'bg-white/[0.04] border-white/[0.06] text-white/70 hover:bg-white/[0.07]'}`}
                    >
                      {copied ? <Check size={14} /> : <Copy size={14} />}
                    </button>
                  </div>
                  <p className="text-white/25 text-[12px] leading-relaxed">{t.sessClientHint}</p>
                </div>
